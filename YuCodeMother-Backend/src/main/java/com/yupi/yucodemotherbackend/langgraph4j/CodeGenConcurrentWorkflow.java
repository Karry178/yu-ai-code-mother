...
import com.yupi.yucodemotherbackend.langgraph4j.node.concurrent.*;
...
/**
 * 【并发方案2 -> LangGraph4j 并发实现：将每个图片收集工具定义为一个工作节点，这些节点可以并发执行】并发执行的代码生成工作流
 */
@Slf4j
public class CodeGenConcurrentWorkflow {
	...
	public WorkflowContext executeWorkflow(String originalPrompt) {
		...
		// 【备注】LangGraph4j旧版本不支持并发，即使按要求写代码，并发分支依旧是串行执行；从1.6.0-rc2版本后，才支持配置线程池
		// -> 因此需要配置线程池和运行时配置
			// 配置并发执行
		ExecutorService pool = ExecutorBuilder.create()
				...
				.setThreadFactory(ThreadFactoryBuilder.create().setNamePrefix("Parallel-Image_Collect").build())
				.build();
	 ...
	 * 路由函数：根据质检结果决定下一步
	 */
	private String routeAfterQualityCheck(MessagesState state) {
